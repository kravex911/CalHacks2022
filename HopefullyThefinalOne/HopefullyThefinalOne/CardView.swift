...
//
//  Created by Umar Hassan on 2022-02-20.
//
...
struct CardContent: View {
    public var body: some View {
        ...
        ZStack {
            ...
            SlideOverView {
                VStack {
                    Text("Slide Over View").font(.title)
                    Text("Slide Over View").font(.title)
                    Spacer()
                }
            }
...
struct CardView: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ZStack(alignment: .top) {
            ZStack(alignment: .top) {
                RoundedRectangle(cornerRadius: 2.5)
                    .frame(width: 40, height: 6.0)
                    .foregroundColor(Color.secondary)
                    .padding(10)
                content.padding(.vertical, 30.0)
            }
        ...
        .shadow(radius: 10)
        .offset(y:  max(0, position + self.dragTracker.height) - 50)
